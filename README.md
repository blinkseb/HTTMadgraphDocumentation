 ...
 * Download Madgraph. You need v2.2.1 at least. Check https://launchpad.net/mg5amcnlo to see if latest version is greater or equals than 2.1.3. If not, install Madgraph using bazaar like this:
```
bzr branch lp:~maddevelopers/mg5amcnlo/2.1.3 MG5_aMC_v2_1_3
```
Otherwise, download from website and extract to the current directory. In the following, the version of Madgraph used may be different than yours: adapt the path to your configuration!

 * Configure Madgraph to use our LHAPDF:
```
cd MG5_aMC_v2_1_3/input
```
 ...
 * Download and install DECAY (replace <username> by the username you use to connect to this wiki)
```
cd MG5_aMC_v2_1_3/
curl -k -L -u <username> https://lyosvn.in2p3.fr/cms_top/raw-attachment/wiki/TTH_Madgraph/DECAY.tar.bz2 -c cookies.txt > /dev/null
curl -k -O -L -u <username> https://lyosvn.in2p3.fr/cms_top/raw-attachment/wiki/TTH_Madgraph/DECAY.tar.bz2 -b cookies.txt
rm cookies.txt
cd DECAY
make clean
make -j5
...
cd ../..
curl -k -L -u <username> https://lyosvn.in2p3.fr/cms_top/raw-attachment/wiki/TTH_Madgraph/runDecay.sh -c cookies.txt > /dev/null
curl -k -O -L -u <username> https://lyosvn.in2p3.fr/cms_top/raw-attachment/wiki/TTH_Madgraph/runDecay.sh -b cookies.txt
rm cookies.txt
chmod +x runDecay.sh
```
 * There is a patch to apply not yet included in the official distribution. Edit the file 'madgraph/various/combine_runs.py' and apply the following patch (you need to add the lines starting with a '+' to the file, you have some context around to see where to put the modifications ; below line 143). It's possible that the patch is already applied to the file if you use a version more recent than 2.1.3.
```
...
The next step is to install the topBSM model into Madgraph. Go to the Madgraph folder (replace <username> by the username you use to connect to this wiki):
```
cd models/
curl -k -L -u <username> https://lyosvn.in2p3.fr/cms_top/raw-attachment/wiki/TTH_Madgraph/topBSM_v4.tar.bz2 -c cookies.txt > /dev/null
curl -k -O -L -u <username> https://lyosvn.in2p3.fr/cms_top/raw-attachment/wiki/TTH_Madgraph/topBSM_v4.tar.bz2 -b cookies.txt
rm cookies.txt
